# shared/fal_lang_profiles.py — Target language / dialect profiles (mirrors languages.js + Modal Edge TTS)
"""
Maps UI language codes (e.g. ar-eg, en-gb) to:
  - Arabic dialect names for translation (same strings as languages.js)
  - Regional neural voices (Edge-style IDs → PlayHT / ElevenLabs parameters)
"""
...
# ── Arabic dialect names (languages.js) ─────────────────────────────────────
...
# Microsoft Edge neural voices per exact lang code (extends routes/tts.py + regional Arabic)
...
# PlayHT v3 voice labels (multilingual; Arabic entries use PlayHT Arabic-capable presets)
...
# ElevenLabs multilingual v2 — voice names (accent influenced by language_code + copy)
...
def resolve_target_language_profile(
    ...
    """
    Resolve full target profile from UI lang code (e.g. ar-eg) — mirrors getLanguageConfig() in languages.js.
    """
    ...
    playht = _lookup_voice(
        _PLAYHT_VOICE_BY_LANG,
        lang_code,
        base,
        "Dexter (English (US)/American)",
    )
    eleven = _lookup_voice(
        _ELEVEN_VOICE_BY_LANG,
        lang_code,
        base,
        "Rachel",
    )
