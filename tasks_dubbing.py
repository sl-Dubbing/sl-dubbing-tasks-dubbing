# tasks_dubbing.py — V2.1 (Production Ready)
import os
import time
import logging
import tempfile
...
# تهيئة Celery لهذا العامل تحديداً
celery_app = make_celery_app('tasks-dubbing', queue_name=QUEUE_DUBBING)

# تهيئة Flask للوصول لقاعدة البيانات
    ...
    if 'runpod.ai' in backend_url or 'runpod.io' in backend_url:
        ...
        while True:
            ...
            if status_data.get('status') in ['FAILED', 'CANCELLED']:
                raise Exception("RunPod Backend failed")
            time.sleep(5)
    else:
        # منطق Modal أو Local (Sync HTTP)
...
def process_dub(self, job_id, user_id, media_url, file_key, lang, cost=100, **kwargs):
    """🚀 المهمة الأساسية"""
    with flask_app.app_context():
        ...
        try:
            ...
            tx = CreditTransaction(user_id=str(user_id), amount=-cost, reason=f'Dubbing: {lang}', job_id=job_id, job_type='dub')
            db.session.add(tx)
            db.session.commit()
